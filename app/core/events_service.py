...
class EventsService:
    ...
    async def update_event_by_id(self, new_data: dict, event_id: UUID) -> Event:
        res = await self.events_repository.update_one(
            new_data, event_id
        )
        return res
...
#what all except get will return dont know
    async def get_events(self,**kwargs):
            ...
            return await self.events_repository.get_many(kwargs.get("limit"),kwargs.get("offset"),
                                                         self.parse_sort(kwargs.get("sort")),data)
#norm esli tut ili kak 

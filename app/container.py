...
class Container(DeclarativeContainer):
    ...
    crypto_service = providers.Singleton(CryptoService,websocket_service=websocket_service)
    websocket_router = providers.Singleton(WebsocketRouter,crypto_service=crypto_service)
    ...
    registration_service = providers.Factory(RegistrationsService,registrations_repository=registrations_repository)
    feedbacks_repository = providers.Factory(FeedbacksRepository, db=db)
    feedbacks_service = providers.Factory(FeedbacksService,feedbacks_repository=feedbacks_repository)
    events_service = providers.Factory(EventsService,events_repository=events_repository,query_builder=query_builder)
    auth_service = providers.Factory(AuthService,users_repository=users_repository,jwt_service=jwt_service)
    attendee_access_controller = providers.Factory(AttendeeAccessController,feedbacks_repository=feedbacks_repository,auth_service=auth_service,registrations_repository=registrations_repository)
    organizer_access_controller = providers.Factory(OrganizerAccessController,events_repository=events_repository,auth_service=auth_service)
    auth_router= providers.Singleton(AuthRouter,auth_service=auth_service,users_service=users_service)
    registration_router = providers.Singleton(RegistrationsRouter,email_service=email_service,registration_service=registration_service,users_service=users_service,attendee_access_controller=attendee_access_controller,auth_service=auth_service)
    healthcheck_router = providers.Singleton(HealthcheckRouter)
    feedback_router = providers.Singleton(FeedbacksRouter,feedbacks_service=feedbacks_service,auth_service=auth_service,attendee_access_controller=attendee_access_controller)
    event_router = providers.Singleton(EventsRouter,events_service=events_service,auth_service=auth_service,organizer_access_controller=organizer_access_controller)
    sms_service = providers.Factory(SmsService, config=config, events_repository=events_repository)
    api_handler = providers.Singleton(APIHandler,events_router=event_router,logger_service=logger_service,websocket_router=websocket_router,feedback_router=feedback_router,healthcheck_router=healthcheck_router,registrations_router=registration_router,scheduler_service=sms_scheduler_service,auth_router=auth_router,sms_service=sms_service)
